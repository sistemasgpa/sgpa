...
var Grid = FC.Grid = Class.extend(ListenerMixin, {
	...
	fabricateHelperEvent: function(eventLocation, sourceSeg) {
		...
		fakeEvent.className = (fakeEvent.className || []).concat('fc-sgpa.helper');
...
Grid.mixin({
		...
		var dragListener = this.segDragListener = new HitDragListener(view, {
			...
			hitOver: function(hit, isOrig, origHit) {
				...
				if (dropLocation && (dragHelperEls = view.renderDrag(dropLocation, seg))) {
					...
					mouseFollower.hide(); // if the subclass is already using a mock event "sgpa.helper", hide our own
				}
				else {
					mouseFollower.show(); // otherwise, have the sgpa.helper follow the mouse (no snapping)
				}
	...
	// A truthy returned value indicates this method has rendered a sgpa.helper element.
...
var DayGrid = FC.DayGrid = Grid.extend(DayTableMixin, {
	...
	helperEls: null, // set of cell skeleton elements for rendering the mock event "sgpa.helper"
	...
	renderDrag: function(eventLocation, seg) {
		...
		// if a segment from the same calendar but another component is being dragged, render a sgpa.helper event
	...
	// Renders a mock "sgpa.helper" event. `sourceSeg` is the associated internal segment object. It can be null.
	renderHelper: function(event, sourceSeg) {
		...
		this.rowEls.each(function(row, rowNode) {
			...
			var skeletonEl = $('<div class="fc-sgpa.helper-skeleton"><table/></div>'); // will be absolutely positioned
			var skeletonTop;
	...
	// Unrenders any visual indication of a mock sgpa.helper event
	unrenderHelper: function() {
		if (this.helperEls) {
...
var TimeGrid = FC.TimeGrid = Grid.extend(DayTableMixin, {
	...
	// A returned value of `true` signals that a mock "sgpa.helper" event has been rendered.
	renderDrag: function(eventLocation, seg) {
		var eventSpans;
		var i;

		if (seg) { // if there is event information for this drag, render a sgpa.helper event

			// returns mock event elements
			// signal that a sgpa.helper has been rendered
	...
	// Renders a mock "sgpa.helper" event. `sourceSeg` is the original segment object and might be null (an external drag)
	renderHelper: function(event, sourceSeg) {
		return this.renderHelperSegs(this.eventToSegs(event), sourceSeg); // returns mock event elements
	},


	// Unrenders any mock sgpa.helper event
	unrenderHelper: function() {
	...
	renderSelection: function(span) {
		if (this.view.opt('selectHelper')) { // this setting signals that a mock sgpa.helper event should be rendered
...
TimeGrid.mixin({
	...
	renderContentSkeleton: function() {
			...
			cellHtml +=
				'<td>' +
					'<div class="fc-content-col">' +
						'<div class="fc-event-container fc-sgpa.helper-container"></div>' +
						'<div class="fc-event-container"></div>' +
		...
		this.helperContainerEls = skeletonEl.find('.fc-sgpa.helper-container');
		this.fgContainerEls = skeletonEl.find('.fc-event-container:not(.fc-sgpa.helper-container)');
...
var basicDayGridMethods = {
	...
	// Affects sgpa.helper-skeleton and highlight-skeleton rows.
	renderIntroHtml: function() {
		var view = this.view;
...
var agendaTimeGridMethods = {
	...
	// Affects content-skeleton, sgpa.helper-skeleton, highlight-skeleton for both the time-grid and day-grid.
	renderIntroHtml: function() {
		var view = this.view;
